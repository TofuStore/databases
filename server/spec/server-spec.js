...
describe('Persistent Node Chat Server', () => {
    ...
    axios.post(`${API_URL}/users`, { username })
      ...
      .then(() => {
        ...
        dbConnection.query(queryString, queryArgs, (err, results) => {
          ...
          expect(results[0].textField).toEqual(message);
          done();
        });
      })
      .catch((err) => {
  ...
  it('Should output all messages from the DB', (done) => {
    ...
    const username = 'Trevor&Kenneth';
    const message = 'Hello';
    const roomname = 'room51';
    const queryString = 'INSERT INTO messages (username, textField, roomname) VALUES (?, ?, ?)';
    ...
    dbConnection.query(queryString, queryArgs, (err) => {
      ...
      axios.get(`${API_URL}/messages`)
        .then((response) => {
          ...
          expect(messageLog[1].textField).toEqual(message);
